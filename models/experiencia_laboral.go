...
type ExperienciaLaboral struct {
	...
	EntidadId           int       `orm:"column(entidad_id)"`
	FechaIngreso        time.Time `orm:"column(fecha_ingreso);type(date)"`
	FechaRetiro         time.Time `orm:"column(fecha_retiro);type(date)"`
	HorasLaboradasDia   int       `orm:"column(horas_laboradas_dia)"`
	DiasLaboradosSemana int       `orm:"column(dias_laborados_semana)"`
}
